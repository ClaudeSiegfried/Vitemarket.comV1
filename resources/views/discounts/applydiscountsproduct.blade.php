                ...
                <div class="form-row mt-5">
                    ...
                    <div class="col-md-5">
                        <select class="selectpicker form-control
                            @error('Produit_id') is-invalid @enderror"
                                data-live-search="true" name="Produit_id"
                                required id="Produit_id"
                                title="Select a product">
                            @foreach($Produits as $produit)
                                <option value="{{$produit -> id}}"
                                        @if (($ProductToEdit ?? '')&& $ProductToEdit->id === $produit -> id)
                                        selected
                                    @endif
                                >
                                    <span>
                                        <img class="img-thumbnail rounded-circle object-fit"
                                             {{$route = route('photo.display', $produit -> photo)}}
                                             src="{{$route}}"
                                             alt="">
                                        {{$produit -> libele}}
                                    </span>
        ...
        <div class="accordion container-fluid align-items-center justify-content-between w-100 mh-75 p-0"
            ...
            <div class="container-fluid row align-items-center justify-content-between w-100 mh-75 p-5 mt-1">
                <div class="col-sm-12 p-4 border-left " id="TableTable"
                    ...
                    <h4 class=" text-muted">Discounts to products list
                        <i class="fas fa-angle-down ml-2"></i></h4>
                </div>
            </div>
            <div
                ...
                <div class="col-sm-12 p-4">
                    ...
                    >
                        ...
                        <tbody>
                        @foreach($ProductsWithDiscounts ?? '' as $item)
                            <tr>
                                <td>{{$item->discount()->get()->first()->name ?? ''}}</td>
                                <td><span class="badge badge-dark">{{$item->discount()->get()->first()->value ?? ''}}
                                        %</span></td>
                                <td>
                                    <img class="img-thumbnail rounded-circle object-fit"
                                         {{$route = route('photo.display', $item -> produit_photo)}}
                                         src="{{$route}}"
                                         alt="">
                                    <span class="container text-bold">
                                        {{$item-> produit_name}}
                                    </span>

                                </td>
                                <td>
                                    <form action="{{route('applyDiscount.deletediscountsproduct',$item->id)}}"
                                          method="post">
                                        @csrf
                                        @method('DELETE')
                                        <a href="{{route('applyDiscount.editdiscountsproduct',$item->id)}}"
