            ...
            <tr>
                <td>
                    <img class="img-thumbnail rounded-circle object-fit"
                         @if (!empty($path = (implode($fournisseur->photo()->get()->pluck('path')->toArray()))))
                ...
                <td>{{implode($fournisseur ->user() ->get()->pluck('name')->toArray())}}</td>
                <td>{{implode($fournisseur ->user() ->get()->pluck('email')->toArray())}}</td>
                <td>{{implode($fournisseur ->user() ->get()->pluck('tel')->toArray())}}</td>
                <td>
                    @can('manage-users')
                        <form action="{{route('fournisseur.destroy',$fournisseur -> id)}}" method="post">
                            @csrf
                            @method('DELETE')
                            <a href="{{route('fournisseur.edit',$fournisseur -> id)}}" class="btn btn-outline-success"><i
